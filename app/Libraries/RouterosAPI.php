...
{
    // Pastikan socket tersedia dan terhubung
    if (!$this->socket || !is_resource($this->socket)) {
        // Jika gagal terhubung, beri tahu pengguna dengan JavaScript
        echo "<script type='text/javascript'>
                alert('Koneksi gagal! Pastikan port dan IP VPN benar.');
                window.location.href = '/router/addrouter'; 
              </script>";
        return false; // Kembalikan false jika koneksi gagal
    }

    // Lanjutkan dengan menulis perintah jika koneksi berhasil
    if ($command) {
        $data = explode("\n", $command);
        foreach ($data as $com) {
            $com = trim($com);
            $encodedCommand = $this->encodeLength(strlen($com)) . $com;

            if (fwrite($this->socket, $encodedCommand) === false) {
                echo "<script type='text/javascript'>
                        alert('Gagal menulis data ke socket.');
                        window.location.href = '/router/addrouter';
                      </script>";
                return false;
            }
        ...
        // Tangani parameter kedua (integer atau boolean)
        if (gettype($param2) == 'integer') {
            $tag = '.tag=' . $param2;
            $encodedTag = $this->encodeLength(strlen($tag)) . $tag . chr(0);

            if (fwrite($this->socket, $encodedTag) === false) {
                echo "<script type='text/javascript'>
                        alert('Gagal menulis tag ke socket.');
                        window.location.href = '/router/addrouter';
                      </script>";
                return false;
            }
        } elseif (gettype($param2) == 'boolean') {
            $boolData = $param2 ? chr(0) : '';
            if ($boolData && fwrite($this->socket, $boolData) === false) {
                echo "<script type='text/javascript'>
                        alert('Gagal menulis data boolean ke socket.');
                        window.location.href = '/router/addrouter'; 
                      </script>";
                return false;
            }
    ...
    return false; // Jika $command kosong, kembalikan false
}
